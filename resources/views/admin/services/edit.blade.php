...
.svc-hero{display:flex;align-items:center;justify-content:space-between;gap:14px;margin-bottom:26px;flex-wrap:wrap;}
.svc-hero-left{display:flex;align-items:center;gap:14px;}
...
.btn-preview{display:inline-flex;align-items:center;gap:7px;padding:9px 16px;border-radius:9px;background:#f3f4f6;color:#374151;font-size:13px;font-weight:600;text-decoration:none;border:1px solid #e5e7eb;transition:background .15s;white-space:nowrap;}
...
/* ── Icon picker trigger ── */
.icon-pick-btn{display:flex;align-items:center;gap:10px;width:100%;padding:10px 13px;border:1px solid #d1d5db;border-radius:10px;background:#fff;cursor:pointer;transition:border-color .15s,box-shadow .15s;font-family:inherit;}
...
.icon-pick-preview{width:34px;height:34px;border-radius:8px;background:#fff7ed;border:1px solid #fed7aa;display:flex;align-items:center;justify-content:center;flex-shrink:0;}
...
.ipk-use-btn{display:inline-flex;align-items:center;gap:7px;padding:10px 20px;border-radius:9px;background:#e07820;color:#fff;font-size:13.5px;font-weight:600;font-family:inherit;border:none;cursor:pointer;transition:background .15s;}
.ipk-use-btn:hover{background:#c96a18;}
.ipk-cancel-btn{display:inline-flex;align-items:center;gap:7px;padding:10px 16px;border-radius:9px;background:#f3f4f6;color:#374151;font-size:13.5px;font-weight:500;font-family:inherit;border:1px solid #e5e7eb;cursor:pointer;transition:background .15s;}
.ipk-cancel-btn:hover{background:#e5e7eb;}
...
.field-row{background:#f9fafb;border:1px solid #e5e7eb;border-radius:10px;padding:14px 16px;margin-bottom:10px;display:grid;grid-template-columns:1fr 150px 32px;gap:10px;align-items:start;transition:border-color .15s;cursor:grab;}
.field-row:hover{border-color:#d1d5db;}
.field-row.dragging{opacity:.35;border-style:dashed;}
...
.doc-body{display:grid;grid-template-columns:1fr 1fr;gap:14px;align-items:start;}
@media(max-width:480px){.doc-body{grid-template-columns:1fr;}}
.enc-note{display:flex;align-items:center;gap:5px;font-size:11px;color:#16a34a;margin-top:8px;}
...
.svc-footer{display:flex;gap:10px;align-items:center;margin-top:4px;flex-wrap:wrap;}
...
.btn-svc-secondary{display:inline-flex;align-items:center;gap:6px;padding:12px 18px;border-radius:10px;background:#f3f4f6;color:#374151;font-size:14px;font-weight:500;text-decoration:none;border:1px solid #e5e7eb;transition:background .15s;}
.btn-svc-secondary:hover{background:#e5e7eb;}
...
.m-btn-yes{flex:1;display:flex;align-items:center;justify-content:center;gap:8px;padding:12px 18px;border-radius:10px;background:#e07820;color:#fff;font-size:14px;font-weight:600;font-family:inherit;border:none;cursor:pointer;transition:background .15s;}
.m-btn-yes:hover{background:#c96a18;}
.m-btn-back{flex:1;display:flex;align-items:center;justify-content:center;gap:7px;padding:12px 18px;border-radius:10px;background:#f3f4f6;color:#374151;font-size:14px;font-weight:500;font-family:inherit;border:1px solid #e5e7eb;cursor:pointer;transition:background .15s;}
...
<div class="svc-wrap">
    ...
    <div class="svc-hero">
        <div class="svc-hero-left">
            <div class="svc-hero-icon"><i class="bi bi-pencil"></i></div>
            <div>
                <h2>Edit Service</h2>
                <p>Update fields, documents, pricing and visibility</p>
            </div>
        </div>
        <a href="{{ route('admin.services.show', $service) }}" class="btn-preview">
    ...
    <form id="serviceForm" method="POST" action="{{ route('admin.services.update', $service) }}">
        ...
        <div class="svc-card s1">
            ...
            <div class="fg">
                ...
                <textarea name="description"
                          placeholder="Brief description of what this service covers...">{{ old('description', $service->description) }}</textarea>
            ...
            {{-- Icon Picker ── replaces old text input ── --}}
            <div class="fg" style="margin-bottom:0;">
                <label>Service icon</label>
                <input type="hidden" name="icon" id="iconValue" value="{{ old('icon', $service->icon ?? 'bi-gear') }}">
                <button type="button" class="icon-pick-btn" onclick="openIconPicker()">
                    <div class="icon-pick-preview">
                        <i class="bi {{ old('icon', $service->icon ?? 'bi-gear') }}" id="iconBtnPreview"></i>
                    </div>
                    <div class="icon-pick-label">
                        <span id="iconBtnName">{{ old('icon', $service->icon ?? 'bi-gear') }}</span>
                        <small>Click to change icon</small>
                    </div>
        ...
        <div class="svc-card s2">
            <div class="svc-sec-head">
                ...
                <div><h3>Application form fields</h3><span>Drag rows to reorder · changes apply after saving</span></div>
            </div>
            <div id="fieldsContainer">
                <div class="svc-empty" id="fieldsEmpty" style="display:none;">
        ...
        <div class="svc-card s3">
            ...
            <div id="docsContainer">
                <div class="svc-empty" id="docsEmpty" style="display:none;">
        ...
        <div class="svc-footer">
            <button type="button" class="btn-svc-primary" onclick="openModal()">
                <i class="bi bi-check-lg"></i> Review &amp; Save
            </button>
            <a href="{{ route('admin.services.show', $service) }}" class="btn-svc-secondary">
                <i class="bi bi-eye"></i> Preview
            </a>
            <a href="{{ route('admin.services.index') }}" class="btn-svc-secondary">
    ...
    <div class="ipk-box">
        ...
        <div class="ipk-search-wrap">
            <div class="ipk-search">
                ...
                <input type="text" id="iconSearch" placeholder="Search icons e.g. file, user, home, money..." oninput="filterIcons()">
            </div>
        </div>
        ...
        <div class="ipk-foot">
            ...
            <button type="button" class="ipk-cancel-btn" onclick="closeIconPicker()">Cancel</button>
            <button type="button" class="ipk-use-btn" onclick="applyIcon()">
...
<div class="svc-modal-bg" id="confirmModal">
    <div class="svc-modal">
        <div class="svc-modal-head">
            <div class="svc-modal-ico"><i class="bi bi-pencil-square"></i></div>
            <div>
        ...
        <div class="svc-modal-foot">
            <button class="m-btn-yes" onclick="submitForm()">
                <i class="bi bi-check-lg"></i> Yes, Save Changes
            </button>
            <button class="m-btn-back" onclick="closeModal()">
...
const ALL_ICONS = [
  ...
  {n:'bi-person-vcard-fill',c:'person document'},{n:'bi-gender-male',c:'person'},{n:'bi-gender-female',c:'person'},
...
function buildGrid(icons){
    ...
    grid.innerHTML=icons.map(ic=>{
        ...
        return `<div class="ipk-item ${sel}" onclick="selectIcon('${ic.n}',this)" title="${ic.n}">
            <i class="bi ${ic.n}"></i><span>${label}</span>
        </div>`;
    }).join('');
}
...
function addField(data){
    ...
    div.className='field-row';div.id='f'+n;div.draggable=true;
    div.innerHTML=`
        <div>
...
function addDoc(data){
    ...
    div.innerHTML=`
        ...
        <div class="doc-body">
            ...
            <div>
                <div class="enc-note" style="margin-top:4px;">
